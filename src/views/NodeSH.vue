...
export default {
  ...
  async mounted() {
    const app = this;
    app.nodes = await app.bdcash.returnNodes();
    ...
    let checksums = await axios.get(
      "https://raw.githubusercontent.com/BdcashProtocol/bdcash-nodesh/master/checksum"
    );
  ...
  methods: {
    async checkNodes() {
      ...
      for (let x in app.nodes) {
        ...
        app.$forceUpdate();
        let response = await app.bdcash.get("/wallet/getinfo", node);
